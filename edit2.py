...
def process_frame(frame):
    lower_bound = np.array([35, 100, 100])  # Adjust according to your background color
    upper_bound = np.array([85, 255, 255])  # Adjust according to your background color
...
frames = apply_chroma_key("inference_result\LipSick_Blend.mp4")
fps = int(cv2.VideoCapture("inference_result\LipSick_Blend.mp4").get(cv2.CAP_PROP_FPS))
...
background_video = background_video.subclip(0, explanation_video.duration)

# Use the audio from the explanation video
final_video = CompositeVideoClip([background_video, explanation_video.set_audio(explanation_video.audio)])
